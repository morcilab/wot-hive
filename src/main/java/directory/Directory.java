...
import directory.triplestore.LocalSparql;
import directory.triplestore.RemoteSparql;
...
public class Directory {
    ...
    public static void main(String[] args) {
        ...
        path("/api", () -> {
            path("/things", () -> {
                ...
                exception(SparqlEndpointException.class, SparqlEndpointException.handleRemoteException);
    ...
    private static final void setup() {
        ...
        try {
            ...
            Sparql sparql;
            if(newConfiguration.isEmbeddedTriplestore()) {
                LOGGER.info("Using local triplestore");
                sparql = new LocalSparql();
            } else {
                sparql = new RemoteSparql();
            }
            sparql.init();
            SparqlController.setSqparql(sparql);
            // Apply service configuration only-once

            port(configuration.getService().getPort());
            threadPool(configuration.getService().getMaxThreads(), configuration.getService().getMinThreads(), configuration.getService().getTimeOutMillis());
            
            Config config = ConfigFactory.load("http.conf");
            LOGGER.info("config: "+config.root().render());
